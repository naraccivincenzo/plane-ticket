...
function handleLuggageChanges(event) {
	...
	// Hand luggage
	if (target.classList.contains('hand-luggage')) {
		const details = target.closest('.col-md-4').querySelector('.hand-luggage-details');
		if (details) {
			details.style.display = target.checked ? 'block' : 'none';
		}
	}

	// Checked luggage
	if (target.classList.contains('checked-luggage')) {
		const details = target.closest('.col-md-4').querySelector('.checked-luggage-details');
		if (details) {
			details.style.display = target.checked ? 'block' : 'none';
		}
	}
...
function fetchFlightData(event) {
    ...
    console.log("Parametri ricerca:", { flightNumber, flightDate });
